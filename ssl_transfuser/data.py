...
class CARLA_Data(Dataset):
    ...
    def __init__(self, root, config):
        ...
        self.next_frame_offset = config.next_frame_offset
    ...
    def __len__(self):
        ...
        return len(self.front) - self.next_frame_offset
    ...
    def __getitem__(self, index):
        ...
        seq_next_fronts = self.front[index + self.next_frame_offset]
        seq_next_lefts = self.left[index + self.next_frame_offset]
        seq_next_rights = self.right[index + self.next_frame_offset]
        seq_next_rears = self.rear[index + self.next_frame_offset]
        seq_next_lidars = self.lidar[index + self.next_frame_offset]
        seq_next_x = self.x[index + self.next_frame_offset]
        seq_next_y = self.y[index + self.next_frame_offset]
        seq_next_theta = self.theta[index + self.next_frame_offset]
        ...
        for i in range(self.seq_len):
            ...
            data['next_fronts'].append(torch.from_numpy(np.array(
                scale_and_crop_image(Image.open(seq_next_fronts[i]), scale=self.scale, crop=self.input_resolution))))
            if not self.ignore_sides:
                ...
                data['next_lefts'].append(torch.from_numpy(np.array(
                    scale_and_crop_image(Image.open(seq_next_lefts[i]), scale=self.scale, crop=self.input_resolution))))
                data['next_rights'].append(torch.from_numpy(np.array(
                    scale_and_crop_image(Image.open(seq_next_rights[i]), scale=self.scale, crop=self.input_resolution))))
            if not self.ignore_rear:
                ...
                data['next_rears'].append(torch.from_numpy(np.array(
                    scale_and_crop_image(Image.open(seq_next_rears[i]), scale=self.scale, crop=self.input_resolution))))
            
            lidar_unprocessed = np.load(seq_lidars[i])[...,:3] # lidar: XYZI
            full_lidar.append(lidar_unprocessed)

            next_lidar_unprocessed = np.load(seq_next_lidars[i])[...,:3] # lidar: XYZI
            next_full_lidar.append(next_lidar_unprocessed)
        ...
        ego_next_x = seq_next_x[i]
        ego_next_y = seq_next_y[i]
        ego_next_theta = seq_next_theta[i]
        ...
        for i in range(self.seq_len, self.seq_len + self.pred_len):
            ...
            next_lidar_unprocessed = np.load(seq_next_lidars[i])
            next_full_lidar.append(next_lidar_unprocessed)  
        ...
        for i in range(self.seq_len + self.pred_len):
            ...
            # process only past lidar point clouds, next frame
            if i < self.seq_len:
                # convert coordinate frame of point cloud
                next_full_lidar[i][:,1] *= -1 # inverts x, y
                next_full_lidar[i] = transform_2d_points(next_full_lidar[i], 
                    np.pi/2-seq_next_theta[i], -seq_next_x[i], -seq_next_y[i], np.pi/2-ego_next_theta, -ego_next_x, -ego_next_y)
                next_lidar_processed = lidar_to_histogram_features(next_full_lidar[i], crop=self.input_resolution)
                data['next_lidars'].append(next_lidar_processed)
