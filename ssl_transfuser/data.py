...
class CARLA_Data(Dataset):
    def __init__(self, root, config):
        ...
        for sub_root in tqdm(root, file=sys.stdout):
                ...
                for route in routes:
                    ...
                    for seq in range(num_seq):
                        ...
                        for i in range(self.seq_len):
                            ...
                            lidars.append(
                                route_dir
                                + f"/lidar/{str(seq*self.seq_len+1+i).zfill(4)}.npy"
                            )

                            # position
                            with open(
                                route_dir
                                + f"/measurements/{str(seq*self.seq_len+1+i).zfill(4)}.json",
                                "r",
                            ) as read_file:
            ...
            print(
                "Preloading "
                + str(len(preload_dict.item()["front"]))
                + " sequences from "
                + preload_file
            )
    ...
    def __getitem__(self, index):
        ...
        for i in range(self.seq_len):
                ...
                torch.from_numpy(
                    np.array(
                        scale_and_crop_image(
                            Image.open(seq_fronts[i]),
                            scale=self.scale,
                            crop=self.input_resolution,
                        )
                    )
                )
            )
            data["next_fronts"].append(
                torch.from_numpy(
                    np.array(
                        scale_and_crop_image(
                            Image.open(seq_next_fronts[i]),
                            scale=self.scale,
                            crop=self.input_resolution,
                        )
                    )
            ...
            if not self.ignore_sides:
                ...
                data["next_lefts"].append(
                    torch.from_numpy(
                        np.array(
                            scale_and_crop_image(
                                Image.open(seq_next_lefts[i]),
                                scale=self.scale,
                                crop=self.input_resolution,
                            )
                        )
                ...
                data["next_rights"].append(
                    torch.from_numpy(
                        np.array(
                            scale_and_crop_image(
                                Image.open(seq_next_rights[i]),
                                scale=self.scale,
                                crop=self.input_resolution,
                            )
                        )
            ...
            if not self.ignore_rear:
                ...
                data["next_rears"].append(
                    torch.from_numpy(
                        np.array(
                            scale_and_crop_image(
                                Image.open(seq_next_rears[i]),
                                scale=self.scale,
                                crop=self.input_resolution,
                            )
                        )
        ...
        for i in range(self.seq_len + self.pred_len):
            ...
            if i < self.seq_len:
                ...
                full_lidar[i] = transform_2d_points(
                    full_lidar[i],
                    np.pi / 2 - seq_theta[i],
                    -seq_x[i],
                    -seq_y[i],
            ...
            if i < self.seq_len:
                ...
                next_full_lidar[i] = transform_2d_points(
                    next_full_lidar[i],
                    np.pi / 2 - seq_next_theta[i],
                    -seq_next_x[i],
                    -seq_next_y[i],
                    np.pi / 2 - ego_next_theta,
                    -ego_next_x,
                    -ego_next_y,
                )
                next_lidar_processed = lidar_to_histogram_features(
                    next_full_lidar[i], crop=self.input_resolution
                )
