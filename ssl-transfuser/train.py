...
parser.add_argument('--cross_modal_prediction_loss_coef', type=float, default=1, help='Coefficient of the image-to-lidar-prediction loss and vice versa.')
parser.add_argument('--next_frame_prediction_loss_coef', type=float, default=1, help='Coefficient of the next frame prediction loss.')
...
class Engine(object):
	...
	def train(self):
		...
		for data in tqdm(dataloader_train):
			...
			next_lidars = []
			for i in range(config.seq_len):
				next_fronts.append(next_fronts_in[i].to(args.device, dtype=torch.float32))
				if not config.ignore_sides:
					next_lefts.append(next_lefts_in[i].to(args.device, dtype=torch.float32))
					next_rights.append(next_rights_in[i].to(args.device, dtype=torch.float32))
				if not config.ignore_rear:
					next_rears.append(next_rears_in[i].to(args.device, dtype=torch.float32))
				next_lidars.append(next_lidars_in[i].to(args.device, dtype=torch.float32))
			...
			pred_wp, image_to_lidar_prediction, lidar_to_image_prediction, next_frame_lidar_prediction, next_frame_image_prediction = model(fronts+lefts+rights+rears, lidars, target_point, gt_velocity)
